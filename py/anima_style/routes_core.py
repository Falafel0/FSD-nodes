...
def register_core_routes(server, require_local_token=None):
    ...
    async def update_artists(request):
        denied = require_local_token(request) if require_local_token else None
    ...
    async def download_images(request):
        # Public endpoint — images are from a public CDN, no auth needed
        started = artist_data.start_image_download()
