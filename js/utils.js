...
import { SITE_BASE, CDN_BASE } from "./config.js";
...
export function thumbUrl(artist, useCustom = false) {
    ...
    const localUrl = `/anima/images/${page}/${id}.webp`;
    // GitHub Pages is the original source — most reliable
    const siteUrl = `${SITE_BASE}/images/${page}/${id}.webp`;
    // statically CDN as secondary cache
    const cdnUrl = `${CDN_BASE}/images/${page}/${id}.webp`;

    // Prefer local when explicitly flagged (image was cached)
    const preferLocal = !!(artist?._preferLocalThumb || artist?.localPreviewCached);
    if (preferLocal) {
        return localUrl;
    }

    // Offline mode: only use local
    const isOnline = localStorage.getItem("anima_online") === "true";
    if (!isOnline) {
        return localUrl;
    }

    // Online mode: use GitHub Pages (most reliable)
    return siteUrl;
}

/**
 * Returns primary + fallback URLs for an artist thumbnail.
 * Primary: GitHub Pages (original source, most reliable).
 * Fallback chain on error: statically CDN → local.
 */
export function thumbUrlPair(artist, useCustom = false) {
    if (!artist) return { primary: "", fallback: "" };
    const id = artist.id ?? "";
    if (!id) return { primary: "", fallback: "" };

    if (useCustom) {
        return { primary: `/anima/images/custom/${id}.webp`, fallback: "" };
    }
    ...
    if (preferLocal || !isOnline) {
        // Primary local, fallback chain: site → CDN
        return { primary: localUrl, fallback: isOnline ? siteUrl : "" };
    }
    // Primary: GitHub Pages (most reliable), fallback: CDN
    return { primary: siteUrl, fallback: cdnUrl };
}
