...
class GlobalMultiLanguageManager {
     ...
     * @param {object} translations - Translation dictionary { ru: {...}, en: {...} }
     */
    ...
    t(key, defaultNamespace = null) {
        ...
        if (!langTranslations) {
            // Fallback to English
            const enTranslations = nsTranslations['en'];
            if (enTranslations) {
                return this.getNestedValue(enTranslations, actualKey) || key;
            }
            return key;
        }
    ...
    getAvailableLanguages() {
        ...
        return Array.from(languages).map(lang => ({
            code: lang,
            name: lang === 'ru' ? 'Русский' : lang === 'en' ? 'English' : lang
        }));
    }
