...
export function createStyleCard({
    ...
    const onErrorJs = fallbackUrl
        ? `var f=this.getAttribute('data-fallback');if(f&&this.src!==f){this.src=f;this.removeAttribute('data-fallback');}else{this.style.display='none';this.parentElement.classList.add('no-img');}`
        : `this.style.display='none';this.parentElement.classList.add('no-img');`;
    const fbAttr = fallbackUrl ? ` data-fallback="${escapeHtml(fallbackUrl)}"` : "";

    card.innerHTML = `
        <div class="anima-card-img" data-init="${escapeHtml((artist.tag?.[0] || "?").toUpperCase())}">
            <img loading="lazy" src="${escapeHtml(imageUrl)}" alt="${escapeHtml(artist.tag || "")}" onerror="${onErrorJs}"${fbAttr}/>
            ${rankHtml}
