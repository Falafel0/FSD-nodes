...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name === "DanbooruGalleryNode" || nodeData.name === "FSD_DanbooruGalleryPipe") {
            ...
            nodeType.prototype.onNodeCreated = function () {
                ...
                const loadLanguage = async () => {
                    try {
                        ...
                        const data = await response.json();
                        globalMultiLanguageManager.setLanguage(data.language || 'en', true);
                    } catch (e) {
                        logger.warn("加载语言设置失败:", e);
                        globalMultiLanguageManager.setLanguage('en', true);
                    }
                };
                ...
                const showSettingsDialog = (initialState = {}) => {
                    ...
                    const ruButton = createLanguageButton('ru', 'Русский');
                    const enButton = createLanguageButton('en', 'English');

                    languageOptions.appendChild(ruButton);
                            ...
                            radioGroup.querySelectorAll('input[type="radio"]').forEach(radio => {
                                ...
                                if (label) {
                                    ...
                                    if (radio.checked) {
                                        ...
                                        label.style.setProperty('border-color', '#2196F3', 'important');
                                        if (indicator) {
                                            indicator.style.setProperty('background-color', '#2196F3', 'important');
                                            indicator.style.setProperty('border-color', '#2196F3', 'important');
                                        }
                                    } else {
                ...
                const showEditPanel = (post) => {
                    ...
                    if (targetWrapper) {
                        ...
                        const selection = {
                            prompt: prompt,
                            image_url: imageUrl,
                            post_id: postToUse.id,
                            post_url: `https://danbooru.donmai.us/posts/${postToUse.id}`,
                            rating: postToUse.rating || '',
                            score: postToUse.score ?? 0,
                            fav_count: postToUse.fav_count ?? 0,
                            created_at: postToUse.created_at || '',
                            image_width: postToUse.image_width ?? 0,
                            image_height: postToUse.image_height ?? 0,
                            source: postToUse.source || '',
                            tags_by_category: {
                                general: postToUse.tag_string_general || '',
                                artist: postToUse.tag_string_artist || '',
                                copyright: postToUse.tag_string_copyright || '',
                                character: postToUse.tag_string_character || '',
                                meta: postToUse.tag_string_meta || '',
                            },
                        };
                    ...
                    const closePanel = (wasSelectedOnOpen) => { // 接收打开面板时的选中状态
                        ...
                        if (wasSelectedOnOpen) {
                            ...
                            const selection = {
                                ...
                                post_url: `https://danbooru.donmai.us/posts/${postToUpdate.id}`,
                                rating: postToUpdate.rating || '',
                                score: postToUpdate.score ?? 0,
                                fav_count: postToUpdate.fav_count ?? 0,
                                created_at: postToUpdate.created_at || '',
                                image_width: postToUpdate.image_width ?? 0,
                                image_height: postToUpdate.image_height ?? 0,
                                source: postToUpdate.source || '',
                                tags_by_category: {
                                    general: postToUpdate.tag_string_general || '',
                                    artist: postToUpdate.tag_string_artist || '',
                                    copyright: postToUpdate.tag_string_copyright || '',
                                    character: postToUpdate.tag_string_character || '',
                                    meta: postToUpdate.tag_string_meta || '',
                                },
                            };
                            ...
                            if (originalPostData) {
                                ...
                                if (isPostCurrentlySelected) {
                                    ...
                                    const selection = {
                                        prompt: prompt,
                                        image_url: imageUrl,
                                        post_id: originalPostData.id,
                                        post_url: `https://danbooru.donmai.us/posts/${originalPostData.id}`,
                                        rating: originalPostData.rating || '',
                                        score: originalPostData.score ?? 0,
                                        fav_count: originalPostData.fav_count ?? 0,
                                        created_at: originalPostData.created_at || '',
                                        image_width: originalPostData.image_width ?? 0,
                                        image_height: originalPostData.image_height ?? 0,
                                        source: originalPostData.source || '',
                                        tags_by_category: {
                                            general: originalPostData.tag_string_general || '',
                                            artist: originalPostData.tag_string_artist || '',
                                            copyright: originalPostData.tag_string_copyright || '',
                                            character: originalPostData.tag_string_character || '',
                                            meta: originalPostData.tag_string_meta || '',
                                        },
                                    };
                                    if (nodeInstance && nodeInstance.widgets) {
                                        const selectionWidget = nodeInstance.widgets.find(w => w.name === "selection_data");
                    ...
                    const saveStyleButton = $el("button", {
                        innerHTML: `💾 ${t('saveAsStyle') || 'Save as Style'}`,
                        title: 'Save selected tags as a Style Selector style',
                        style: {
                            padding: "8px 15px",
                        ...
                        onclick: async () => {
                            try {
                                const { showSaveStyleModal } = await import("./index.js");

                                // Get the current post data (possibly edited)
                                const postData = temporaryTagEdits[post.id] || post;

                                // Collect selected categories from checkboxes
                                const selectedCategoriesCheckboxes = panel.querySelectorAll('.danbooru-edit-category-checkbox:checked');
                                const selectedCategories = Array.from(selectedCategoriesCheckboxes).map(cb => cb.name);
                                ...
                                // Collect tags from selected categories
                                const tags = [];
                                categoriesToUse.forEach(category => {
                                    const tagStr = postData["tag_string_" + category];
                                    if (tagStr) {
                                        tagStr.split(" ").forEach(tag => {
                                            let processed = tag;
                                            if (replaceUnderscores) processed = processed.replace(/_/g, " ");
                                            if (escapeBrackets) processed = processed.replace(/\(/g, "\\(").replace(/\)/g, "\\)");
                                            tags.push(processed);
                                        });
                                    }
                                });
                                ...
                                console.error("[DanbooruGallery] Failed to open save style modal:", err);
                                showToast("Failed to open save dialog", "error", saveStyleButton);
                            }
                        }
                    });
                    editPanelFooter.appendChild(saveStyleButton);
                    ...
                    selectedWrappers.forEach(wrapper => {
                        ...
                        if (postData) {
                            ...
                            selections.push({
                                ...
                                prompt: prompt,
                                image_url: imageUrl,
                                post_url: `https://danbooru.donmai.us/posts/${postId}`,
                                rating: postData.rating || '',
                                score: postData.score ?? 0,
                                fav_count: postData.fav_count ?? 0,
                                created_at: postData.created_at || '',
                                image_width: postData.image_width ?? 0,
                                image_height: postData.image_height ?? 0,
                                source: postData.source || '',
                                tags_by_category: {
                                    general: postData.tag_string_general || '',
                                    artist: postData.tag_string_artist || '',
                                    copyright: postData.tag_string_copyright || '',
                                    character: postData.tag_string_character || '',
                                    meta: postData.tag_string_meta || '',
                                },
                            });
                        }
                    });
...
$el("style", {
   ...
   .danbooru-category-checkbox:checked::before {
    ...
    .danbooru-filter-button.active { background-color: #2196F3; color: white; border-color: #2196F3; }
    ...
    .danbooru-image-wrapper.selected {
        border-color: #2196F3; /* A more vibrant purple */
    ...
    .danbooru-image-buttons button:hover {
        ...
        background-color: rgba(255, 255, 255, 0.9) !important; /* 统一使用紫色悬浮效果 */
    }
    ...
    .danbooru-status.error { color: #f55; }
        ...
        .danbooru-blacklist-dialog textarea:focus { outline: 2px solid #2196F3; }
...
$el("style", {
    ...
    .danbooru-edit-panel-content button:hover {
        background-color: rgba(255, 255, 255, 0.2); /* 统一紫色悬浮效果，增强视觉强度 */
        border-color: #2196F3;
        color: white; /* 悬浮时文字改为白色，提高对比度 */
        transform: translateY(-2px);
    }
    .danbooru-edit-panel-content button:active {
        background-color: rgba(255, 255, 255, 0.3); /* 点击时进一步加深 */
        transform: translateY(0);
    }
...
$el("style", {
    ...
    .danbooru-edited-indicator {
        ...
        top: 5px;
        left: 5px;
        background-color: rgba(255, 255, 255, 0.9); /* 紫色背景 */
        color: white;
        padding: 3px 8px;
        font-size: 10px;
        font-weight: bold;
